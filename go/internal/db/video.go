...
func UpdateVideoFile(v VideoData) error {
	_, err := db.Exec(`
		...
		ON CONFLICT(path) DO UPDATE SET
			...
			size = CASE WHEN size IS NULL OR size = 0 THEN excluded.size ELSE size END,
			duration = CASE WHEN duration IS NULL OR duration = 0 THEN excluded.duration ELSE duration END,
			year = CASE WHEN year IS NULL OR year = 0 THEN excluded.year ELSE year END,
			...
			season = CASE WHEN season IS NULL OR season = 0 THEN excluded.season ELSE season END,
			episode = CASE WHEN episode IS NULL OR episode = 0 THEN excluded.episode ELSE episode END,
